...
interface CardType3Props {
  product: Product;
  onSelectProduct?: (product: Product, selections: Record<string, any>, price: number | null) => void;
}

export function CardType3SizeCustomQty({ product, onSelectProduct }: CardType3Props) {
  ...
  const currentSelections = { ...selections, quantity: customQty.toString() };
  ...
  const handleQuantityChange = (newVal: number) => {
    const validVal = Math.max(1, Math.min(999, newVal || 1));
    setCustomQty(validVal);
  };
        ...
        <div className="p-5 flex flex-col flex-1 justify-between gap-4">
          ...
          <div className="p-3 bg-[#F7F7F5] border border-[#E3E3DE] rounded-2xl space-y-2">
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => handleQuantityChange(customQty - 1)}
                disabled={customQty <= 1}
                className="w-9 h-9 rounded-full bg-white border border-[#E3E3DE] flex items-center justify-center text-[#0E0F08] hover:bg-[#CC0000] hover:text-white hover:border-[#CC0000] transition-colors disabled:opacity-40 cursor-pointer"
              >
              ...
              <button
                type="button"
                onClick={() => handleQuantityChange(customQty + 1)}
                className="w-9 h-9 rounded-full bg-white border border-[#E3E3DE] flex items-center justify-center text-[#0E0F08] hover:bg-[#CC0000] hover:text-white hover:border-[#CC0000] transition-colors cursor-pointer"
              >
...
export default CardType3SizeCustomQty;
